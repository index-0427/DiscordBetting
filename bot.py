...
async def on_application_command_error(ctx, error):
    logging.error(f"Command error in /{ctx.command.name}: {error}")
    try:
        await ctx.respond(f"エラーが発生しました: {error}", ephemeral=True)
    except:
        pass
...
@bot.slash_command(name='bet-start', description='賭けを開始 管理者専用')
...
@bot.slash_command(name='bet-close', description='賭けを中断する 管理者のみ')
async def close(ctx):
    if not await check_admin(ctx): return
    if not globalDict:
...
@bot.slash_command(name='bet-winner', description='試合の勝者を決定 管理者のみ')
...
@bot.slash_command(name='bet-refund', description='全てのポイントを返金 管理者のみ')
...
@bot.slash_command(name='bet-addpt', description='ポイントを増やす 管理者のみ')
...
@bot.slash_command(name='bet-reducept', description='ポイントを減らす 管理者のみ')
...
@bot.slash_command(name='bet-leaderboard', description='1ポイント以上持っている人の一覧と総額 管理者のみ')
...
@bot.slash_command(name='bet-ptlog', description='addpt/reduceptの利用履歴と統計 管理者のみ')
async def ptlog(ctx):
    if not await check_admin(ctx): return
...
@bot.slash_command(name='bet-balance', description='特定のユーザーのポイントを確認する 管理者のみ')
